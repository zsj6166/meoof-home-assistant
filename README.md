...
觅凹联网自动喂食器的 Home Assistant 自定义集成。通过手机号验证码登录，发现账号下的设备，并提供设备状态、喂食与觅食历史、实时画面、录像下载、猫咪档案以及可选的外部视觉识别。仓库同时内置 Lovelace 卡片，安装集成后无需再复制 JavaScript 文件。
...
安装并重启后，集成会自动加载内置卡片。单台喂食器直接粘贴以下 YAML 即可，不需要知道实体 ID。
...
- 目前主要在一款双仓觅凹喂食器与 Linux amd64 HA 环境验证，其他型号需要社区反馈。
- 只公开经过验证的左侧出粮控制；右侧控制不在 UI 中提供。
