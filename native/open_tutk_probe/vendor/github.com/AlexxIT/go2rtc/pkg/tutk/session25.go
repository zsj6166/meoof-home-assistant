...
func NewSession25(conn net.Conn, sid []byte) *Session25 {
	base := NewSession16(conn, sid)
	// TUTK/29 devices expect the SDK's 0x1b client wire revision. This also
	// selects the compatible RDT channel negotiation used for file transfer.
	base.wireVersion = 0x1b
	return &Session25{
		Session16: base,
		rb:        NewReorderBuffer(5),
		cmdChunks: make(map[uint32]*commandChunks),
	}
}
...
type Session25 struct {
	*Session16
	...
	cmdChunks map[uint32]*commandChunks
...
type commandChunks struct {
	total  uint16
	chunks map[uint16][]byte
}

const cmdHdrSize25 = 28
const maxIOCtrlChunk25 = 1024

func (s *Session25) SendIOCtrl(ctrlType uint32, ctrlData []byte) []byte {
	return s.SendIOCtrlChunks(ctrlType, ctrlData)[0]
}

// SendIOCtrlChunks fragments a logical IOCTRL message into the 1024-byte
// TUTK/29 command chunks used by the camera firmware. The four-byte control
// type is part of the logical message and therefore only appears in chunk 0.
func (s *Session25) SendIOCtrlChunks(ctrlType uint32, ctrlData []byte) [][]byte {
	data := make([]byte, 4+len(ctrlData))
	binary.LittleEndian.PutUint32(data, ctrlType)
	copy(data[4:], ctrlData)

	total := (len(data) + maxIOCtrlChunk25 - 1) / maxIOCtrlChunk25
	if total == 0 {
		total = 1
	}
	messageID := s.seqSendMsg
	chunks := make([][]byte, 0, total)
	for index := 0; index < total; index++ {
		start := index * maxIOCtrlChunk25
		end := start + maxIOCtrlChunk25
		if end > len(data) {
			end = len(data)
		}
		part := data[start:end]
		size := msgHhrSize + cmdHdrSize25 + uint16(len(part))
		msg := s.Msg(size)

		// 0  0070      command
		// 2  0b00      version
		// 4  1000      packet sequence
		// 6  0076      ???
		cmd := msg[msgHhrSize:]
		...
		// 10 0300      chunk sequence
		// 12 0300      chunks count
		// 14 0000      chunk index (starts from 0)
		// 16 0004      chunk size (maximum 1024)
		// 18 0000      random msg id (always 0)
		// 20 03000000  logical message id (constant for all chunks)
		// 24 00000000
		cmd[9] = 0x70
		binary.LittleEndian.PutUint16(cmd[10:], s.seqSendCmd2+uint16(index))
		binary.LittleEndian.PutUint16(cmd[12:], uint16(total))
		binary.LittleEndian.PutUint16(cmd[14:], uint16(index))
		wireSize := len(part)
		if total == 1 {
			// Existing TUTK/29 devices count the final four reserved command
			// bytes for an unfragmented client command. Preserve that wire
			// quirk; fragmented continuations use their exact payload size.
			wireSize += 4
		}
		binary.LittleEndian.PutUint16(cmd[16:], uint16(wireSize))
		binary.LittleEndian.PutUint16(cmd[20:], messageID)
		copy(cmd[28:], part)
		chunks = append(chunks, msg)
	}

	s.seqSendCmd2 += uint16(total)
	s.seqSendMsg++
	return chunks
}
...
func (s *Session25) SessionRead(chID byte, cmd []byte) (res int) {
	...
	case 0x00:
		if len(cmd) >= 12 && cmd[1] == 0x70 {
			// TUTK/29 carries the received command sequence at offset 10.
			// Using the protocol-version field at offset 2 leaves the command
			// unacknowledged and makes the device retransmit it indefinitely.
			s.seqRecvCmd2 = binary.LittleEndian.Uint16(cmd[10:])
		}
...
// recvCommandChunk adds support for IOCTRL payloads larger than one TUTK/29
// datagram. Upstream originally exposed every continuation as an independent
// command, which truncated Meoof's 2178-byte feed-plan response.
func (s *Session25) recvCommandChunk(cmd []byte) {
	if len(cmd) < 28 {
		return
	}
	total := binary.LittleEndian.Uint16(cmd[12:])
	if total <= 1 {
		s.queueCommand(cmd[28:])
		return
	}
	messageID := binary.LittleEndian.Uint32(cmd[20:])
	index := binary.LittleEndian.Uint16(cmd[14:])
	if index >= total {
		index &= 0x1f
	}
	if index >= total {
	...
	state := s.cmdChunks[messageID]
	if state == nil || state.total != total {
		state = &commandChunks{total: total, chunks: make(map[uint16][]byte)}
		s.cmdChunks[messageID] = state
	}
	state.chunks[index] = bytes.Clone(cmd[28:])
	debugf("command chunk message=%d index=%d total=%d bytes=%d", messageID, index, total, len(cmd[28:]))
	if len(state.chunks) != int(total) {
		return
	}
	var complete []byte
	for chunk := uint16(0); chunk < total; chunk++ {
		data, ok := state.chunks[chunk]
		if !ok {
			return
		}
		complete = append(complete, data...)
	}
	delete(s.cmdChunks, messageID)
	debugf("command complete message=%d bytes=%d", messageID, len(complete))
	s.queueCommand(complete)
}

func (s *Session25) queueCommand(command []byte) {
	select {
	case s.rawCmd <- bytes.Clone(command):
	default:
		debugf("command queue full, dropped %d bytes", len(command))
	}
}
