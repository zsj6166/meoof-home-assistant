...
type Session interface {
	...
	SendChannelData(chID byte, payload []byte) error
...
func (s *Session16) SessionWrite(chID byte, buf []byte) error {
	...
	case 1:
		...
		buf[14] = 1 // channel
	case 3:
		binary.LittleEndian.PutUint16(buf[6:], s.seqSendCh3)
		s.seqSendCh3++
		buf[14] = 3
	}
...
func (s *Session16) SendChannelData(chID byte, payload []byte) error {
	message := s.Msg(uint16(msgHhrSize + len(payload)))
	copy(message[msgHhrSize:], payload)
	return s.SessionWrite(chID, message)
}
